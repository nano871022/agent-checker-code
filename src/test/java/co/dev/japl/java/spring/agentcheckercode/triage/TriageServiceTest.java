...
class TriageServiceTest {
    ...
    @BeforeEach
    void setUp() {
        triageService = new TriageService(gitHubClientService, crashAnalysisAiService, "co.dev.japl.java.spring");
    }
    ...
    void triageCrashEvent_internalPackage_delegatesToAiService() {
        ...
        StackFrame internalFrame = StackFrame.builder()
                .packageName("co.dev.japl.java.spring.agentcheckercode.service")
                .className("co.dev.japl.java.spring.agentcheckercode.service.UserService")
                .build();
        ...
        CrashEvent crash = CrashEvent.builder()
                ...
                .stackTrace("at co.dev.japl.java.spring.agentcheckercode.service.UserService.processUser(UserService.java:45)")
                .build();

        TriageResult result = triageService.triageCrashEvent("owner", "repo", crash);

        assertThat(result.getDecision()).isEqualTo(TriageDecision.RESOLVABLE_BY_AGENT);
        verify(crashAnalysisAiService).analyzeCrash(contains("co.dev.japl.java.spring.agentcheckercode.service.UserService"));
    }
